...
g_sCmdDesc = "Starts Blender for debugging in a given workspace configuration"
...
def AddArgParseArguments(_parseArgs):
    ...
    _parseArgs.add_argument("--port", nargs=1, dest="port", default=["5678"])
    _parseArgs.add_argument("-w", "--wait", nargs=1, dest="wait", default=["10"])
...
def RunCmd(_argsCmd, _lArgs):
    ...
    from . import debug_impl as impl
    ...
    fWaitSeconds: float = None
    if argsSubCmd.wait[0] is not None:
        try:
            fWaitSeconds = float(argsSubCmd.wait[0])
        except Exception:
            raise RuntimeError(f"Wait time is not a number: {argsSubCmd.wait[0]}")
        # endtry
    # endif
    ...
    dicArgs["sPort"] = argsSubCmd.port[0]
    dicArgs["fWaitSeconds"] = fWaitSeconds
