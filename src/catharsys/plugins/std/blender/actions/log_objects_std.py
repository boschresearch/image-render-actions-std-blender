...
# -*- coding:utf-8 -*-
###
# File: /do-log-objects-std.py
# Created Date: Friday, October 23rd 2020, 7:22:06 am
...
def GetDefinition():
    return {
        ...
        "sActionDTI": "/catharsys/action/std/blender/render/log:1.0",
...
def Run(_xCfg: CConfigList):
    ...
    from .lib.cls_log_obj import CLogObj

    ####################################################################################
    def Log(_xPrjCfg, _dicCfg, **kwargs):

        iCfgIdx = kwargs.get("iCfgIdx")
        iCfgCnt = kwargs.get("iCfgCnt")
        if iCfgIdx is None or iCfgCnt is None:
            raise CAnyError_Message(sMsg="Render action parameters 'iCfgIdx' and 'iCfgCnt' not specified")
        # endif

        xRender = CLogObj(xPrjCfg=_xPrjCfg, dicCfg=_dicCfg)
        xRender.Init()
    ...
    _xCfg.ForEachConfig(Log)


# enddef

################################################################################
def ResultData(xJobCfg: CConfigManifestJob) -> None:
    return None
